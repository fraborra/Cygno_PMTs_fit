                ...
                const std::vector<double>& L4_inp, const std::vector<double>&x, const std::vector<double>& y): BCModel(mode)
{
	...
	AddParameter("alpha",0.1,6.,"#alpha","");
    ...
    for(unsigned int i=0; i<nPoints; i++) { // i == nPoint index
        ...
        for(unsigned int j=0; j<4; j++) { // j == PMT index
            ...
            LL += BCMath::LogGaus(Lij,                            // x, namely Lj
                                 (pars[0]*pars[k])/(pow(tmp, 0.5*pars[-1])), // mu, namely the light computed in the step (c_i * Lj / r_i^4)
                                 sLij,                            // sigma
                                 true                             // norm factor
                                 );
        }
...
// Define prior
double PMTfindalpha::LogAPrioriProbability(const std::vector<double>& pars) {
    double LL = 0.;
