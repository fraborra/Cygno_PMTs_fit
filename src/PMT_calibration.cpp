...
PMTcalibration::PMTcalibration(const std::string &mode, int nth, int nP,
    ...
    if (mode_ == "PMTcalibration") {
        ...
        GetParameter("L").Fix(
            4000.0); // just to have c_i values smaller, can put any value,
...
void runCalibrationFit(const Config config) {
    ...
    std::string BAT_out_prefix_cal =
        res_dir + cal.GetSafeName() + "_" + output_tag;
    ...
    if (plot) {
        ...
        cal.PrintCorrelationMatrix(BAT_out_prefix_cal +
