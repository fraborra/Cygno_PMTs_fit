...
PMTfindalpha::PMTfindalpha(const std::string &mode, int nth, int nP,
                           ...
                           const std::vector<double> &y, double *c_tmp)
    : PMTcalibration() {
...
void runFindAlphaFit(const Config config) {
    ...
    for (int point = start_ind; point < end_ind; point++) {
        ...
        L1.push_back(data.getL1()[point] / data.getSc_integral()[point] *
                     10000);
        L2.push_back(data.getL2()[point] / data.getSc_integral()[point] *
                     10000);
        L3.push_back(data.getL3()[point] / data.getSc_integral()[point] *
                     10000);
        L4.push_back(data.getL4()[point] / data.getSc_integral()[point] *
                     10000);
    ...
    std::string BAT_out_prefix_alpha =
        res_dir + alpha.GetSafeName() + "_" + output_tag;
    ...
    if (plot) {
        ...
        alpha.PrintCorrelationMatrix(BAT_out_prefix_alpha +
